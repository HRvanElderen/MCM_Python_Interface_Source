...
// Vector with layer information for operators already used (i.e. already affected to a layer):
  vector< unsigned int > vect_op_used(NOp_tot+1, n);  // initialisation  //all operator not already used have "layer = n" (it's the last layer);
...
//********* All other basis operators:  phi_i:
  ...
  while( (it_Op != allOp_data.end()) && (basis.size() < n) )
  {
    //search for the next basis operator:
    while(vect_op_used[(*it_Op).bin] < n)
